...
const CONTRACT_ADDRESS = 'ST164QV81BRK296G6RTV6G1ZH208FQ36QHEPJ3K61'
...
export async function getGame(gameId: number) {
  ...
  const game: Game = {
    ...
    'player-two': gameCV['player-two'].type === 'some' ? gameCV['player-two'].value.value : null,
    ...
    winner: gameCV['winner'].type === 'some' ? gameCV['winner'].value.value : null
  }
  return game
}

export async function createNewGame(betAmount: number, moveIndex: number, move: Move) {
  const txOptions = {
    contractAddress: CONTRACT_ADDRESS,
    contractName: CONTRACT_NAME,
