...
export function GamesList({ games }: { games: Game[] }) {
  ...
  const userGames = useMemo(() => {
    ...
    const filteredGames = games.filter(
      (game) =>
        (game['player-one'] === userAddress || game['player-two'] === userAddress) &&
        game.winner === null
    )
  ...
  const joinableGames = useMemo(() => {
    ...
    return games.filter(
      (game) =>
        game.winner === null && game['player-one'] !== userAddress && game['player-two'] === null
    )
  }, [games, userData])
          ...
          {userGames.length === 0 ? (
            ...
            <div className="flex items-center gap-8 max-w-7xl overflow-y-scroll">
              {userGames.map((game, index) => (
                ...
                >
                  <GameBoard key={index} board={game.board} cellClassName="size-8 text-xl" />
